  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <select
        value={value?.id ?? ''}
        onChange={(e) => {
          const m = mice.find((x) => x.id === e.target.value);
          if (m) onChange(m);
        }}
...
export default function VisualizerPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:py-12">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-[#e8e8ed] sm:text-4xl">Shape Visualizer</h1>
        <p className="mt-2 text-[#9a9aab]">Overlay and compare mouse shapes from different views.</p>
      </div>

      {/* Selectors */}
      <div className="grid gap-4 sm:grid-cols-2">
        <MouseSelector label="Mouse 1" value={mouse1} onChange={setMouse1} />
        <MouseSelector label="Mouse 2" value={mouse2} onChange={setMouse2} />
      </div>

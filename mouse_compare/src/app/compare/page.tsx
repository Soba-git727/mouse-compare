    ...
    <div ref={ref} className="relative">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-[#2a2a3a] bg-[#12121a] py-2.5 pl-10 pr-10 text-sm text-[#e8e8ed] outline-none placeholder:text-[#6a6a7a] focus:border-[#6c5ce7]/50 disabled:cursor-not-allowed disabled:opacity-50"
        />
        {selected.length < 4 && (
          <ChevronDown className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#6a6a7a]" />
        )}
      </div>
        ...
        <div className="absolute left-0 right-0 top-full z-40 mt-1 max-h-60 overflow-auto rounded-lg border border-[#2a2a3a] bg-[#12121a] shadow-xl scrollbar-thin">
          {filtered.length === 0 ? (
            ...
            filtered.map((m) => (
              ...
              >
                <div className="relative h-8 w-8 shrink-0 overflow-hidden rounded-md bg-[#1a1a26]">
...
// Mouse selection modal component
function MouseSelectionModal({ selected, onSelect, onRemove, onClose }: {
  selected: MouseSpec[];
  onSelect: (m: MouseSpec) => void;
  onRemove: (id: string) => void;
  onClose: () => void;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div ref={ref} className="bg-[#0a0a0f] rounded-xl border border-[#2a2a3a] w-full max-w-md max-h-[80vh] overflow-hidden">
        <div className="p-4 border-b border-[#2a2a3a]">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-[#e8e8ed]">Select Mice</h2>
            <button
              onClick={onClose}
              className="text-[#6a6a7a] hover:text-[#e8e8ed] transition-colors"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          <div className="relative">
            ...
            <input
              ...
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className="w-full rounded-lg border border-[#2a2a3a] bg-[#12121a] px-3 py-2 pl-10 text-sm text-[#e8e8ed] outline-none placeholder:text-[#6a6a7a] focus:border-[#6c5ce7]/50"
            />
          </div>
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((m) => (
              <button
                ...
                className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#e8e8ed] transition-colors hover:bg-[#1a1a26]"
              >
                <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-md bg-[#1a1a26]">
                  <img src={m.photo} alt={m.name} className="block h-full w-full object-cover" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate font-medium">{m.name}</div>
        ...
        <div className="p-4 border-t border-[#2a2a3a]">
          <div className="text-xs text-[#6a6a7a] mb-2">
            Selected: {selected.length}/4 mice
          </div>
          <button
            onClick={onClose}
            className="w-full rounded-lg bg-[#6c5ce7] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#5a4bd6]"
          >
            Done
          </button>
        </div>
...
export default function ComparePage({
  ...
  const { preselect } = use(searchParams);

  useEffect(() => {
    if (preselect) {
      const mouse = mice.find((m) => m.id === preselect);
      if (mouse) {
        setSelected([mouse]);
      }
    }
                  ...
                  {selected.map((m) => (
                    <th key={m.id} className="min-w-[180px] px-4 py-4">
                      <div className="flex flex-col items-center gap-2 text-center">
                        <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-xl bg-[#1a1a26]">
                          <img src={m.photo} alt={m.name} className="block h-full w-full object-cover" />
                        </div>
                        <div>
      ...
      {selected.length === 0 && (
        <div className="mt-16 text-center">
          ...
          <button
            ...
            className="mt-4 rounded-lg bg-[#6c5ce7] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#5a4bd6]"
          >
            Select Mice
          </button>
        </div>

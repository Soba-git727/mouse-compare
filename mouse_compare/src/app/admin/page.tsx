...
export default function AdminPage() {
  ...
  const [adminReviews, setAdminReviews] = useState<{ id: string; userName: string; mouseName: string; text: string; rating: number; createdAt: string }[]>([]);
  ...
  useEffect(() => {
    ...
    fetch('/api/reviews').then(r => r.json()).then(d => setAdminReviews(d.reviews || [])).catch(() => {});
  }, [router]);
          ...
          <>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
              {stats.map((stat) => {
                ...
                const displayValue = stat.label === 'Total Accounts' ? String(userCount) : stat.value;
                return (
                  <div
                    key={stat.label}
          ...
          <div className="space-y-6">
            <div className="rounded-xl border border-[#2a2a3a] bg-[#12121a]">
              <div className="border-b border-[#2a2a3a] px-5 py-4">
                <h3 className="text-sm font-semibold text-[#e8e8ed]">User Reviews ({adminReviews.length})</h3>
              </div>
              <div className="overflow-x-auto">
                    ...
                    <tbody>
                      {adminReviews.map((review) => (
                        <tr key={review.id} className="border-b border-[#2a2a3a] last:border-0 hover:bg-[#1a1a26]/50 transition-colors">
                          ...
                          <td className="px-5 py-3 text-xs text-[#9a9aab] max-w-xs truncate">{review.text}</td>
                          <td className="px-5 py-3">
                            <button

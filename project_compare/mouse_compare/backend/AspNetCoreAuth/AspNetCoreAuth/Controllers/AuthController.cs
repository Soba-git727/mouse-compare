...
public class AuthController : ControllerBase
{
    ...
    public async Task<ActionResult<AuthResponse>> Register([FromBody] RegisterRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.Name) ||
            ...
            return BadRequest(new AuthResponse { Success = false, Error = "Missing required fields" });
        ...
        if (existing != null)
            return BadRequest(new AuthResponse { Success = false, Error = "Email already registered" });
        ...
        if (!result.Succeeded)
            return BadRequest(new AuthResponse { Success = false, Error = "Registration failed" });
    ...
    public async Task<ActionResult<AuthResponse>> Login([FromBody] LoginRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.Email) || string.IsNullOrWhiteSpace(request.Password))
            return BadRequest(new AuthResponse { Success = false, Error = "Missing email or password" });
    ...
    public async Task<ActionResult<AuthResponse>> Me()
    {
        ...
        if (string.IsNullOrEmpty(token))
            return Unauthorized(new AuthResponse { Success = false, Error = "Not authenticated" });
        ...
        try
        {
            ...
            var role = principal.FindFirst(ClaimTypes.Role)?.Value.ToLower() ?? "user";

...
function MouseSpecDropdown({ miceList, selected, onSelect, onRemove }: DropdownProps) {
...
export default function ComparePage({
  ...
  const [allMice, setAllMice] = useState<MouseSpec[]>([]);
  ...
  useEffect(() => {
    if (preselect) {
      const mouse = allMice.find((m) => m.id === preselect);
      if (mouse) {
        setSelected([mouse]);
      }
    }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:py-12">
      ...
      <MouseSpecDropdown miceList={allMice} selected={selected} onSelect={handleSelect} onRemove={handleRemove} />

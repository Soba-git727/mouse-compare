...
export default function ReviewsPage() {
  ...
  const loadReviews = useCallback(() => {
    ...
    if (filterMouse) params.set('mouseId', filterMouse);
    ...
    fetch(`/api/reviews?${params.toString()}`)
      ...
      .then(d => {
        setServerReviews(d.reviews || []);
        setReviewTotal(d.total ?? 0);
      })
      .catch(() => setReviewsError('Could not load reviews. Please try again.'))
      .finally(() => setReviewsLoading(false));
  }, [filterMouse, reviewPage]);
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !formMouse || !formText.trim()) return;
    setSubmitting(true);
    try {
  ...
  const handleUpdateReview = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setServerReviews(prev => prev.map(r => (r.id === id ? data.review : r)));
        cancelEdit();
      }
    } finally {
      setSubmitting(false);
        ...
        <form onSubmit={handleSubmitReview} className="mb-8 rounded-xl border border-[#2a2a3a] bg-[#12121a] p-5 space-y-4">
          ...
          <div>
            ...
            >
              ...
              {allMice.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </div>
          <div>
      ...
      <section>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h2 className="flex items-center gap-2 text-lg font-semibold text-[#e8e8ed]">
            ...
            Latest Reviews
            {filterMouse && <span className="text-sm font-normal text-[#6a6a7a]">
              · {serverReviews.length} for “{allMice.find(m => m.id === filterMouse)?.name ?? filterMouse}”
            </span>}
          </h2>
          <select
            value={filterMouse}
            onChange={e => { setFilterMouse(e.target.value); setReviewPage(1); }}
            className="rounded-lg border border-[#2a2a3a] bg-[#12121a] px-3 py-2 text-sm text-[#e8e8ed] outline-none focus:border-[#6c5ce7]/50"
          >
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {reviewsLoading ? (
            ...
            <p className="text-sm text-[#6a6a7a]">No reviews yet{filterMouse ? ' for this mouse' : ''}.</p>
          ) : (
          serverReviews.map((review, idx) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 text-sm font-medium text-[#e8e8ed]">
                      {review.userName}
                      {review.userId === user?.id && <span className="rounded bg-[#6c5ce7]/15 px-1.5 py-0.5 text-[10px] font-semibold text-[#6c5ce7]">YOU</span>}
                ...
                <div className="flex items-center gap-3">
                  {review.userId === user?.id && editingId !== review.id && (
              ...
              {editingId === review.id ? (
                <div className="space-y-3">
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full rounded-lg border border-[#2a2a3a] bg-[#0a0a0f] px-3 py-2 text-sm text-[#e8e8ed] outline-none placeholder:text-[#6a6a7a] focus:border-[#6c5ce7]/50"
                  />
                  <div className="flex items-center gap-2">
                    <button
                    ...
                    <button
                      onClick={cancelEdit}
                      className="flex items-center gap-1.5 rounded-lg border border-[#2a2a3a] px-3 py-1.5 text-xs font-medium text-[#9a9aab] hover:bg-[#1a1a26] hover:text-[#e8e8ed]"
                    >
          ...
          {!reviewsLoading && sampleReviews.map((review, idx) => (
            <div
              key={review.id}
              className="animate-fade-slide-up rounded-xl border border-[#2a2a3a] bg-[#12121a] p-5 transition-all hover:border-[#6c5ce7]/30"
        ...
        {reviewTotal > 9 && (
          <div className="mt-8 flex items-center justify-center gap-2">
            ...
            <span className="px-3 text-sm text-[#6a6a7a]">
              Page {reviewPage} of {Math.max(1, Math.ceil(reviewTotal / 9))}
            </span>
            <button
              onClick={() => setReviewPage((p) => Math.min(Math.ceil(reviewTotal / 9), p + 1))}
              disabled={reviewPage >= Math.ceil(reviewTotal / 9)}
              className="flex items-center gap-1 rounded-lg border border-[#2a2a3a] px-3 py-2 text-sm text-[#9a9aab] transition-colors hover:bg-[#1a1a26] hover:text-[#e8e8ed] disabled:cursor-not-allowed disabled:opacity-40"
            >
